...
#include <geometry_msgs/Pose/Point.h>
#include <geometry_msgs/Pose/Quaternion.h>
...
////ROS global vars
std_msgs::Int16 lw;
ros::Publisher lwheel_chatter("lwheel", &lw);
std_msgs::Int16 rw;
ros::Publisher rwheel_chatter("rwheel", &rw);
geometry_msgs::Pose imudata[2];//posearray
ros::Publisher imu_chatter("imuArray", imudata); //DEBUG: put & before imudata if error thrown
//ROS global vars
...
float ax, ay, az, ax1, ay1, az1, gx1, gy1, gz1;       // Stores the real accel value in g's
...
void updatetick_right(){
    if(dir_r_avg==1)
    cntr++; 
    else 
    cntr--; 
    
   //publishing rightwheel ticks
   rw.data = cntr;
   rwheel_chatter.publish( &rw );
     
}

void updatetick_left(){
    if(dir_l_avg==1)
    cntl++; 
    else
    cntl--; 
    
   //publishing leftwheel ticks
    lw.data = cntl;
    lwheel_chatter.publish( &lw );
//    Serial.println("Wwheel"+String(cntl*-1));   
}
...
void loop()
{  
//  Serial.println("askfdnaslkfnlwswkfnlaskfn"); 
// detachInterrupt(digitalPinToInterrupt(pin1)); 
//  detachInterrupt(digitalPinToInterrupt(pin3)); 
...
/*IMU**********/
    ///PUBLISHING VALUES OF IMU
    ///PUBLISHING VALUES OF acceleration
//    imudata[0].Point.x =ax1; 
  //  imudata[0].Point.y =ay1; 
    //imudata[0].Point.z =az1; 
    
    ///PUBLISHING VALUES OF gyroscope
    //imudata[1].Point.x =gx1; 
    //imudata[1].Point.y =gy1; 
    //imudata[1].Point.z =gz1;
    
    ///PUBLISHING VALUES OF EULER ANGLES (YPR)
    //imudata[2].Point.x =yaw; 
    //imudata[2].Point.y =pitch; 
    //imudata[2].Point.z =roll; 
    //publishing normalised quaternion on imudata(0)
    //imudata[0].Quaternion.x = q[0];  
    //imudata[0].Quaternion.y = q[1]; 
    //imudata[0].Quaternion.z = q[2]; 
    //imudata[0].Quaternion.w = q[3];  
    // Final publishing statement
    //imu_chatter.publish(imudata);   //add '&' before imudata if error thrown

        //DEBUG TO PRINT IMU VALUES
//    Serial.println(String(ax1)+","+String(ay1)+","+String(az1)+","+String(gx1)+","+String(gy1)+","+String(gz1)+"\n"); 
//    Serial.print(yaw, 2);
//    Serial.print(", ");
//    Serial.print(pitch, 2);
//    Serial.print(", ");
//    Serial.println(roll, 2);    
   // delay(1000); // DEBUG Uncomment the delay here if IMU doesnt work  
...
//AVERAGING OVER 20 time instances      
      if(ll>=0)
     left_dir[--ll]=dirl=="fwd"?1:0;
      else {
      ll=5; 
      for (i=0;i<ll; i++)
       sl+=left_dir[i]; 
 dir_l_avg = (sl/5)>=0.5?1:0; //1 for forward 0 for rvs
// attachInterrupt(digitalPinToInterrupt(pin1),updatetick_left , RISING); //fwd

//Serial.println(dir_l_avg); 
 //ticks
 sl=0; 
 }
...
///RIGHT ENCODER
  ...
  if(digitalRead(pin3) && digitalRead(pin4))//11
  ...
  else if(!digitalRead(pin3) && digitalRead(pin4)){ //01
  ...
  else if(digitalRead(pin3) && !digitalRead(pin4)){ //10 
      if(stater==2)
      dirr = "fwd";
      else if(stater==0)
      dirr = "bwd"; 
  ...
  else if(!digitalRead(pin3) && !digitalRead(pin4)){ //00
      if(stater==3)
      dirr = "fwd";
      else if(stater==1)
      dirr = "bwd"; 
...
//AVERAGING OVER 20 time instances      
      if(lr>=0)
     right_dir[--lr]=dirr=="fwd"?1:0;
      else {
      lr=5; 
      for (i=0;i<lr; i++)
       sr+=right_dir[i]; 
 dir_r_avg = (sr/5)>=0.5?1:0; //1 for forward 0 for rvs
// attachInterrupt(digitalPinToInterrupt(pin1),updatetick_right , RISING); //fwd
...
void getAres() {
  ...
  // 2 Gs (00), 4 Gs (01), 8 Gs (10), and 16 Gs  (11). 
        // Here's a bit of an alugorith to calculate DPS/(ADC tick) based on that 2-bit value:
    case AFS_2G:
          aRes = 2.0/32768.0;
          break;
    case AFS_4G:
