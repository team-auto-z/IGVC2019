...
//AVERAGING OVER 20 time instances      
     ...
     left_dir[--ll]=dirl=="fwd"?1:0;
      ...
      for (i=0;i<ll; i++)
       sl+=left_dir[i]; 
     //dir
//     cntl = ((sl/50)>=0.5)?cntl+1:cntl-1; 
 dir_l_avg = (sl/50)>=0.5?"fwd ":"rvs ";
 Serial.print(dir_l_avg); 
 //ticks
 Serial.println(cntl); 
 sl=0; 
 }
...
//AVERAGING OVER 20 time instances      
     ...
     right_dir[--lr]=dirr=="fwd"?1:0;
      ...
      for (i=0;i<lr; i++)
       sr+=right_dir[i]; 
     //dir
//     cntr = (sr/50)>=p0.5?cntr+1:cntr-1; 
  dir_r_avg = (sr/50)>=0.5?"fwd ":"rvs ";
